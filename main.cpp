...
const char kWindowTitle[] = "LD2B_06_ナガトモイチゴ_MT3_04_03";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	conicalPendulum.angularVelo = 0.0f;
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (play) {
			conicalPendulum.angularVelo = std::sqrt(9.8f / (conicalPendulum.length * std::cos(conicalPendulum.halfApexAngle)));
			conicalPendulum.angle += conicalPendulum.angularVelo * deltaTime;
		}
		...
		DrawLine({conicalPendulum.anchor, ball} , projectionMatrix , viewportMatrix , WHITE);
